...
func css*(w:Webview; css: cstring):cint{.discardable.} = 
    w.js( fmt"console.log(1);document.addEventListener('DOMContentLoaded', () => {CSS_INJECT_FUNCTION}({$css}));".cstring )
...
func dispatch(w: Webview; fn: pointer; arg: pointer) {.importc: "webview_dispatch", header: headerC.} # dispatch nim func,function will be executed on the UI thread
